...
//    LOGV("Overlay2 configuration: %d*%d@%d-bit, smem_len=%d\n", var.xres, var.yres, var.bits_per_pixel, fix.smem_len);
	...
	printf("yoff %d, ylen %d cboff %d cblen %d croff %d, crlen %d fixleng %d step %d\n",pDispCfg->yoff,pDispCfg->ylen,pDispCfg->cboff,pDispCfg->cblen,pDispCfg->croff,pDispCfg->crlen,pDispCfg->fix_smem_len,pDispCfg->step);

	printf("format %d, bpp %d screen_width %d, screen_height %d, step %d height %d\n",
		   pDispCfg->format, pDispCfg->bpp, pDispCfg->screen_width, pDispCfg->screen_height,
		   pDispCfg->step, pDispCfg->height);
...
//    LOGV("base fb configuration: %d*%d@%d-bit, nonstd=%x\n", var_base.xres, var_base.yres, var_base.bits_per_pixel, var_base.nonstd);
	...
	printf("%p %p %p \n", pDispCfg->overlay[0],pDispCfg->overlay[1],pDispCfg->overlay[2]);
